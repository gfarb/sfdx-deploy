...
class ZipWriter extends ComponentWriter {
    ...
    async _write(chunk, encoding, callback) {
        let err;
        try {
            await Promise.all(chunk.writeInfos.map(async (writeInfo) => this.addToZip(chunk.component.type.folderType ?? chunk.component.type.folderContentType
                ...
                : // everything else can be zipped immediately
                    await (0, exports.stream2buffer)(writeInfo.source), writeInfo.output)));
        }
        catch (e) {
            err = e;
        }
